...
class AlertUtilityTestCase(unittest.TestCase):
    ...
    def _set_all_records(self, inadequate="inade", forbidden="forbid"):
        api.portal.set_registry_record(
            name="inadequate_words", interface=IStopWords, value=inadequate
        )
        api.portal.set_registry_record(
            name="forbidden_words", interface=IStopWords, value=forbidden
        )
    ...
    def test_no_forbidden_registry_no_error(self):
        ...
        self.assertEqual(self.utility._get_registry_stop_words(), "")
    ...
    def test_empty_registry_no_error(self):
        ...
        self.assertEqual(self.utility.get_snippets("some random text"), "")

    def test_has_words_from_empty_registry(self):
        """Check that if the registry is empty has_stop_words returns False."""
        self._set_record_value("")
        self.assertFalse(self.utility.has_stop_words("some random text"))

    def test_has_words_from_registry(self):
        """Check that has_stop_words works with the registry."""
        self._set_record_value("random\nalert me\nlala")
        self.assertTrue(self.utility.has_stop_words("some random text"))

    def test_no_has_words_from_registry(self):
        """Check that has_stop_words works with the registry."""
        self._set_record_value("random\nalert me\nlala")
        self.assertFalse(self.utility.has_stop_words("some specific text"))
    ...
    def test_no_has_forbidden_words(self):
        ...
        """
        self._set_record_value("random\nalert me\nlala", record="forbidden_words")
        self.assertFalse(self.utility.has_forbidden_words("some specific text"))
    ...
    def test_get_snippets_from_registry(self):
        ...
        self._set_record_value("random\nalert me\nlala")
        self.assertEqual(
            self.utility.get_snippets("some random text", chars=2),
            "random\n\n...e random t...",
        )
    ...
    def test_no_get_snippets_from_registry(self):
        """Check that get_snippets works with the registry."""
        self._set_record_value("random\nalert me\nlala")
        self.assertEqual(self.utility.get_snippets("some specific text"), "")
    ...
    def test_get_all_normalized_stop_words(self):
        ...
        self.assertIn("forbid", normalized)
        self.assertIn("inade", normalized)
    ...
    def test_get_forbidden_registry_normalized_stop_words(self):
        ...
        self._set_all_records()
        normalized = self.utility.get_normalized_stop_words(register="forbidden_words")
        self.assertIn("forbid", normalized)
        self.assertNotIn("inade", normalized)
    ...
    def test_get_inadequate_registry_normalized_stop_words(self):
        ...
        """
        self._set_all_records()
        normalized = self.utility.get_normalized_stop_words(register="inadequate_words")
        self.assertNotIn("forbid", normalized)
        self.assertIn("inade", normalized)
    ...
    def test_has_forbidden_words_no_text(self):
        ...
        self.assertFalse(self.utility.has_forbidden_words(""))
        self.assertFalse(self.utility.has_forbidden_words(None))
    ...
    def test_has_forbidden_words_no_words_in_registry(self):
        ...
        """
        self.assertIsNone(
            api.portal.get_registry_record(interface=IStopWords, name="forbidden_words")
        )
        self.assertFalse(self.utility.has_forbidden_words("lalala"))
    ...
    def test_has_forbidden_words_no_words_found(self):
        ...
        api.portal.set_registry_record(
            interface=IStopWords, name="forbidden_words", value="one\ntwo"
        )

        self.assertFalse(self.utility.has_forbidden_words("no words found"))
    ...
    def test_has_forbidden_words_words_found(self):
        ...
        self.assertTrue(self.utility.has_forbidden_words("and now one is found!"))
    ...
    def test_has_inadequate_words_words_found(self):
        ...
        """
        api.portal.set_registry_record(
            interface=IStopWords, name="inadequate_words", value="one\ntwo"
        )

        self.assertTrue(self.utility.has_inadequate_words("and now one is found!"))
...
class HTMLNormalizeTestCase(unittest.TestCase):
    ...
    def test_lower_case(self):
        text = "UAU"
        self.assertEqual(alert_text_normalize(text), "uau")

    def test_unicode_normalized_form(self):
        text = "älert"
        self.assertEqual(alert_text_normalize(text), "alert")
    ...
    def test_html_entity(self):
        text = "alert&#220;s"
        self.assertEqual(alert_text_normalize(text), "alertus")
    ...
    def test_multiple_spaces_on_source(self):
        text = "alert     text"
        self.assertEqual(alert_text_normalize(text), "alert text")
    ...
    def test_string_umlauts(self):
        text = "some \xfc"
        self.assertEqual(alert_text_normalize(text), "some u")

    def test_string_other_unicode_chars(self):
        text = "s\ufe05o\ufe06m\ufe05e"
        self.assertEqual(alert_text_normalize(text), "some")
...
class GetSnippetsTestCase(unittest.TestCase):
    ...
    def test_no_text(self):
        text = None
        stop_words = "one\ntwo"
        self.assertEqual(self.snippets(text, stop_words), "")
    ...
    def test_no_stop_word_in_text(self):
        text = "Random normal text"
        stop_words = "one\ntwo"
        self.assertEqual(self.snippets(text, stop_words), "")

    def test_stop_word_in_text(self):
        text = "Alerts two text"
        stop_words = "one\ntwo"
        snippet_text = self.snippets(text, stop_words, chars=1)
        self.assertEqual(snippet_text, "two\n\n... two ...")

    def test_unicode_text(self):
        text = "Alerts twö text"
        stop_words = "one\ntwo"
        snippet_text = self.snippets(text, stop_words, chars=3)
        self.assertEqual(snippet_text, "two\n\n...ts two te...")

    def test_multiple_stop_words(self):
        text = "Alerts one text and two more text"
        stop_words = "one\ntwo"
        snippet_text = self.snippets(text, stop_words, chars=1)
        self.assertEqual(snippet_text, "one, two\n\n... one ...\n\n... two ...")

    def test_different_line_endings(self):
        text = "and one alert or second alert and even third alert on text"
        stop_words = "one alert\r\nsecond alert\nthird alert"
        snippet_text = self.snippets(text, stop_words, chars=2)
        self.assertEqual(
            snippet_text,
            "one alert, second alert, third alert"
            "\n\n...d one alert o..."
            "\n\n...r second alert a..."
            "\n\n...n third alert o...",
        )
    ...
    def test_ignore_empty_lines(self):
        text = "and one alert or text"
        stop_words = "one alert\n\n\n\nsecond alert\nthird alert"
        snippet_text = self.snippets(text, stop_words, chars=2)
        self.assertEqual(snippet_text, "one alert\n\n...d one alert o...")

    def test_same_stop_word_more_than_once(self):
        text = "Alerts one text and one more text"
        stop_words = "one\ntwo"
        snippet_text = self.snippets(text, stop_words, chars=3)
        self.assertEqual(snippet_text, "one\n\n...ts one te...\n\n...nd one mo...")
    ...
    def test_keep_text_in_order(self):
        ...
        text.
        """
        text = "Alerts two text and one more text"
        stop_words = "one\ntwo"
        snippet_text = self.snippets(text, stop_words, chars=3)
        self.assertEqual(snippet_text, "two, one\n\n...ts two te...\n\n...nd one mo...")

    def test_keep_text_in_order_multiple_occurrences(self):
        text = "Alerts two text and one more text and some more two tired"
        stop_words = "one\ntwo"
        snippet_text = self.snippets(text, stop_words, chars=2)
        self.assertEqual(
            snippet_text, "two, one\n\n...s two t...\n\n...d one m...\n\n...e two t..."
        )
...
class GetTextFromObjectTest(unittest.TestCase):
    def test_contentish_like(self):
        ...
        self.assertEqual(get_text_from_object(DummyDXComment()), "found!")
    ...
    def test_event_with_contentish_like(self):
        ...
        self.assertEqual(get_text_from_object(DummyEventDX()), "found!")
    ...
    def test_event_with_comment_like(self):
        ...
        self.assertEqual(get_text_from_object(DummyEventComment()), "found!")
...
class VerifyBrainTestCase(unittest.TestCase):
    ...
    def test_no_brain(self):
        ...
        """
        self.assertIsNone(verify_brain(self.doc, new_entries="random"))
    ...
    def test_no_entries(self):
        ...
        catalog = api.portal.get_tool("portal_catalog")
        brain = catalog(id=self.doc.id)[0]
        self.assertIsNone(verify_brain(brain, new_entries=""))
    ...
    def test_none_entries(self):
        ...
        catalog = api.portal.get_tool("portal_catalog")
        brain = catalog(id=self.doc.id)[0]
        self.assertIsNone(verify_brain(brain, new_entries=None))

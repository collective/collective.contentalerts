...
from collective.contentalerts.testing import (  # noqa
    COLLECTIVE_CONTENTALERTS_ASYNC_FUNCTIONAL_TESTING,
)
...
class TestVerifiedInterfaceHandler(unittest.TestCase):
    ...
    def setUp(self):
        ...
        api.portal.set_registry_record(
            name="inadequate_words", interface=IStopWords, value="lala\nlili"
        )
    ...
    def test_new_word_added(self):
        ...
        task = taskqueue.get()
        self.assertIn("size=300", task["url"])
        self.assertIn("start=0", task["url"])
        self.assertIn("entries=magic", task["url"])
    ...
    def test_new_word_with_spaces_added(self):
        ...
        self.assertIn("size=300", task["url"])
        self.assertIn("start=0", task["url"])
        self.assertIn("entries=magic+powers+%26+more", task["url"])

    def test_new_forbidden_word(self):
        """Check that with forbidden words work as well"""
        api.portal.set_registry_record(
            name="forbidden_words", interface=IStopWords, value="lala\nmagic"
        )
        ...
        self.assertIn("size=300", task["url"])
        self.assertIn("start=0", task["url"])
        self.assertIn("entries=lala%0Amagic", task["url"])
